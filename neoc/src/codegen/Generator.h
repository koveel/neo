...
public:
	...
	RValue MaterializeToRValue(const Value& v);
	LValue EmitSubscript(BinaryExpression* expression);
	LValue EmitMemberAccessExpression(BinaryExpression* expression);
	...
	RValue CastRValueIfNecessary(const RValue& rv, Type* to, bool isExplicit, Expression* source);
	...
	// Values
	bool ScopeValue(const std::string& name, const ScopedValue& value);
private:
